...
case class Cache[K, V](underlying: CCache[K, Entry[V]]) extends Service[K, V] {
  override def get(key: K): Option[V] = Try(
    underlying.getIfPresent(key).value
  ).toOption
  override def remove(key: K): Unit = underlying.invalidate(key)
  override def put(key: K, value: V, ttl: Option[Duration]): Unit =
    underlying.put(key, Entry(value, ttl))
  override def toMap: Map[String, String] =
    underlying.asMap().asScala.map(x => (x._1.toString, x._2.value.toString)).toMap
  ...
  override def getStats: Map[String, String] =
    Map(
      ...
      "size"         -> underlying.asMap().size.toString,
